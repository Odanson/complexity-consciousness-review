...
This review will produce that map. It will (i) catalogue every quantitative complexity measure that has been applied to neural data in a consciousness context, (ii) classify the corpus along nine orthogonal axes (mathematical lineage, mathematical primitive, data requirement, temporal granularity, aspect of consciousness, spatial scale, theory anchoring, inferential status, validation evidence), (iii) audit the fit between each measure's mathematical content and the claims made with it, and (iv) flag systematic gaps — in particular the structural geometry of experience and the experience of time.
...
| Protocol version / date | v0.4.2 / 2026-05-12 |
...
- **Summary tables:** (i) measure × mathematical primitive; (ii) measure × mathematical lineage (Axis 0); (iii) measure × data requirement; (iv) measure × consciousness aspect claimed; (v) measure × validation evidence.
...
> **Why nine axes.** v0.4.2 adds **Axis 0 — Mathematical lineage** on Jurgen's observation that *the taxonomies in mathematics and neuroscience are different*: a paper grouping by surface family name (as Axis 1 does) is not the same as a partition by the measure's formal mathematical home. Axis 0 records the latter; Axis 1 records the former; conflating the two produces exactly the conceptual fog this review aims to dispel. Axes 2–8 cover orthogonal application and design dimensions. A given measure carries one entry in each axis (or more, where a measure's genealogy is multi-rooted).

**Axis 0 — Mathematical lineage (added v0.4.2)**

Each measure family maps onto a formal mathematical home — distinct from the phenomenological grouping of Axis 1.

- **Algorithmic information theory.** Lempel-Ziv (LZ76, LZc, LZW); Kolmogorov signal complexity (KSC); compressibility variants.
- **Classical (Shannon) information theory and probability.** Entropy family (Shannon, differential, approximate, sample, permutation, spectral, multiscale); mutual information; transfer entropy; weighted symbolic mutual information (wSMI).
- **Causal / interventionist information theory.** Granger causality; partial directed coherence; φ-family (canonical φ, φ*, φ-AR, φ-SI, φ-CII, φ-C, geometric φ, whole-minus-sum, φ-ID, φ-atomic); causal density; causal emergence; state differentiation.
- **Dynamical-systems theory.** Lyapunov exponents; correlation dimension; chaos / edge-of-chaos indices; phase-locking value (PLV); weighted phase-lag index (wPLI); metastability; chimera-state indices; phase coherence and synchrony.
- **Statistical physics / critical phenomena.** Neural avalanches; branching parameter; long-range temporal correlations (LRTCs); detrended fluctuation analysis (DFA) and multifractal DFA; power-law exponent (PLE); scale-free measures; Hurst exponent; fractal dimension.
- **Algebraic-graph and topological theory.** Small-worldness; modularity; rich-club coefficient; hierarchical complexity; Tononi-Sporns-Edelman 1994 neural complexity; global-signal topography (GS-topography).
- **Differential geometry / manifold methods.** Intrinsic dimensionality; participation ratio; embedding dimension; optimal-transport distances; qualia-space unfolding.
- **Time-series statistics (applied).** Autocorrelation window (ACW); temporal receptive windows (TRW); spectral-content measures.
- **Experimental / perturbational design.** PCI; PCI-state (ST-PCI); PCI-LZW. These are *combinations* of an experimental manipulation (TMS perturbation) and a downstream mathematical operator (e.g., LZc on the TMS-EEG response); they belong here distinctly because the inferential warrant comes from the perturbation, not from the operator alone.
- **Hybrid / supervised statistical learning.** Classifier-based composites whose features are drawn from any of the above; the lineage is statistical-learning theory rather than any single primitive.

A measure may be listed under more than one lineage where the genealogy is multi-rooted (e.g., φ-ID sits at the intersection of integrated information and partial-information decomposition; wSMI sits at the intersection of classical mutual information and symbolic-dynamics permutation methods). The lineage attribution is the *mathematical home* of the measure, not the consciousness-research family that uses it — those map to Axis 1.
...
> Remaining open item: **OSF registration timing** — before vs after the multi-database run. To be decided in the next meeting.
