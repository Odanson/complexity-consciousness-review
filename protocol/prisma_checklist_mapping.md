...
| 14 | Managing records | Total records identified | Multi-database search executed 2026-06-10: **14,866 records** raw, pre-deduplication (PubMed 4,912; Scopus 4,212; Web of Science 3,750; PsycINFO 783; IEEE Xplore 1,209). See `search/search_execution/search_execution_report.md` and the five per-database `search_log.md` files. §B6.1 also retains the v0.4.1 pilot count (PubMed = 5,267 as of 2026-05-12) as historical comparator. |
| 15 | Managing records | Deduplication | §B7 protocol method (Bramer method in Zotero + Rayyan deduplicator + manual spot-check). **Executed Phase 1 (2026-06-15 → 06-16):** Rayyan duplicate-detection over all 14,866 imported records (with the IEEE CSV exports first converted to RIS via `convert_ieee_csv_to_ris.py`); 9,613 duplicate groups detected, 3,293 manually resolved, 35 marked "Not duplicate", **6,285 records deleted → 8,581-record deduplicated corpus**. Workflow + file index recorded in `search/corpus_construction/log.md`; canonical deduplicated corpus at `search/corpus_construction/rayyan_exports/rayyan_dedup_export_2026-06-16/articles.ris`. |
| 16 | Reporting | Date of search | Multi-database locked search executed **2026-06-10** (all five databases within the same 24-hour window); recorded per database in `search/search_execution/<database>/search_log.md` and summarised in `search/search_execution/search_execution_report.md`. §B6.1 retains pilot dates (v0.3 pilot 2026-05-07; v0.4.1 re-pilot 2026-05-12) as historical comparators. |
...
**OSF Registries registration completed:** DOI [10.17605/OSF.IO/FU82J](https://doi.org/10.17605/OSF.IO/FU82J); registration record at [https://osf.io/fu82j/](https://osf.io/fu82j/).

**Multi-database search executed (2026-06-10).** Raw retrieval recorded in `search/search_execution/search_execution_report.md` (14,866 records across PubMed, Scopus, Web of Science, PsycINFO, IEEE Xplore); per-database execution logs in `search/search_execution/<database>/search_log.md`.

**Phase 1 deduplication completed (2026-06-15 → 06-16).** Rayyan duplicate-detection + manual adjudication reduced 14,866 imported references to an **8,581-record deduplicated corpus** (6,285 duplicates deleted; 35 manually marked "Not duplicate"). Workflow + statistics in `search/corpus_construction/log.md`; canonical corpus at `search/corpus_construction/rayyan_exports/rayyan_dedup_export_2026-06-16/articles.ris`. **PRISMA-S items 14, 15, and 16 are now populated;** Phase 2 title / abstract screening on the 8,581-record corpus (Elicit + ASReview per §B7) is the next milestone.
