...
**Version 0.4 — 2026-05-11**

**Lead reviewer:** Samson Odan
**Supervisors:** Jurgen (primary), Lucia, Andrej
**Planned external collaborators (later phase):** Adam Barnett, Anil Seth
...
| Protocol version / date | v0.4 / 2026-05-11 |
...
| Arbiter | Any two of Jurgen, Lucia, Andrej |
...
**Expert elicitation.** Suggestions solicited from Jurgen, Lucia, Andrej, Anil Seth, and Adam Barnett; every suggestion logged together with whether the database search already captured it (this gives a sensitivity estimate).
...
#### B6.1 Pilot results (2026-05-07)

The search was piloted against PubMed via NCBI E-utilities (script: `pilot_search.py`). Block-individual and intersection counts:

| Query | Records |
|---|---:|
| Block 1 (consciousness) | 1,550,317 |
| Block 2 (complexity) | 524,399 |
| Block 3 (neural data) | 1,356,030 |
| **Block 1 ∧ Block 2 ∧ Block 3** | **4,580** |

The intersection sits comfortably inside the target band of 3,000–15,000 records and supports calibrated single-reviewer screening with active-learning prioritisation (per §B7).

#### B6.2 Seed-paper validation

Eight empirical seeds (§B9a) were checked against the locked search; **seven of eight are captured**. The exception — Sitt et al. 2014, *Brain* (PMID 24919971) — is a documented database miss: its title and abstract describe the analytic battery as "neural signatures of consciousness" without naming any specific complexity measure, so it does not match Block 2. Broadening Block 2 to capture this framing (e.g., adding `"neural signature*"`) was rejected because it would inflate noise across all five other databases. Sitt 2014 is recovered through §B5 backward / forward citation tracking on the seed set, which it satisfies via virtually every captured DOC / EEG paper. The miss is recorded in §B9a and re-flagged in any future re-pilot.

The two methodology-only seeds (§B9b — Tononi-Sporns-Edelman 1994; Oizumi-Albantakis-Tononi 2014) are correctly **not** captured by the database search; both are theoretical / derivation papers without neural-data application. They enter the review via the §B4 seminal-methodological-paper exception, not via search, and are validated only by PMID resolution.
...
- Julio's qualia-structure / unfolding work (citation to verify with Lucia).
...
2. Data suitability (sufficient channels / record length for the measure?).
3. Validation (ground truth? cross-validated?).
4. Control for confounds (SNR, sampling-rate differences, muscle artefact, state-dependence of the measure at the zero-information limit).
5. Reproducibility (code + data + parameters).
6. Interpretation (claims commensurate with what the measure computes?).

Each item rated *low / some concern / high*. Piloted on 10 studies, then revised.
...
4. **Authorship policy and order.** To settle before Adam Barnett and Anil Seth are formally invited; provisional order Samson (lead) → Jurgen → Lucia → Andrej, with Adam Barnett and Anil Seth added on contribution.
